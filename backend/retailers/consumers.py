...
class InventoryPredictionConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def send_inventory_predictions(self, event):
        """Processes the actual event logic cleanly."""
        await self.helper_func()
        await self.send_json({
            'status': 'completed', 
    ...
    # CRITICAL MAPPER ANCHOR: Maps the hyphenated string to your Python function
    # Paste this exactly as a key-value attribute inside your class
