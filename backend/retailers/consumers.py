...
class InventoryPredictionConsumer(AsyncJsonWebsocketConsumer):
    ...
    def helper_func(self):
        ...
        p_ids = set(list(RetailerReceipts.objects.filter(owner=self.user, is_active="true").values_list('product_id', flat=True)) + list(OutOfStock.objects.filter(owner=self.user).values_list('product_id', flat=True)))
        compiled = []
        ...
        for p_id in p_ids:
            try:
                ...
                if len(s_list) + len(o_list) < 3: continue
