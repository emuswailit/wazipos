...
import { DrugSubClassItem } from "./index";

interface SubClassDetailsModalProps {
    routeItem: DrugSubClassItem | null;
    onClose: () => void;
    theme: any;
    formatDateHandler: (dateString: string) => string;
    onOpenEditTrigger: (item: DrugSubClassItem) => void;
}

export default function SubClassDetailsModal({ routeItem, onClose, theme, formatDateHandler, onOpenEditTrigger }: SubClassDetailsModalProps) {
                ...
                <View style={{ backgroundColor: theme.panel, borderBottomColor: theme.border }} className="h-16 w-full border-b px-6 flex-row justify-between items-center z-50 shadow-sm">
                    <View className="flex-row items-center gap-x-3">
                        <View style={{ backgroundColor: theme.primary + "15" }} className="px-2.5 py-1 rounded-md">
                            <Text style={{ color: theme.primary }} className="text-[10px] font-black tracking-widest uppercase">Subclass Spec</Text>
                        </View>
                        <Text style={{ color: theme.text }} className="text-base font-black tracking-tight">Subclass Specifications</Text>
                    </View>
                    ...
                    <View className="w-full max-w-2xl mx-auto flex-col gap-y-6">
                        ...
                        <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm items-start w-full flex-col gap-y-4">
                            <View className="w-full">
                                <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-2">Drug Subclass Name</Text>
                        ...
                        <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm w-full flex-row justify-between gap-4 flex-wrap">
                            <View className="items-start min-w-[140px]">
                                <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-1">Parent Drug Class Linkage</Text>
                                <Text style={{ color: theme.text }} className="text-sm font-black bg-slate-500/10 px-3 py-1 rounded-lg mt-2 uppercase tracking-wide self-start">{routeItem?.drug_class_title}</Text>
                            </View>
                        </View>
                        ...
                        <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm items-start w-full">
                            <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-2">Pharmacological Actions & Indications</Text>
                            <Text style={{ color: theme.text }} className="text-sm font-medium leading-relaxed text-left w-full">{routeItem?.description || "No specific clinical indicators submitted for this structural node profile parameter."}</Text>
                        </View>
