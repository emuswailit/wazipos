...
interface SubClassListSectionProps {
    subclasses: DrugSubClassItem[];
    isLargeScreen: boolean;
    theme: any;
    cardBorderClass: string;
    formatDateHandler: (dateString: string) => string;
    onOpenDetailsTrigger: (item: DrugSubClassItem) => void;
    onOpenEditTrigger: (item: DrugSubClassItem) => void;
}

export default function SubClassListSection({
    subclasses,
    isLargeScreen,
    theme,
    cardBorderClass,
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={true} contentContainerClassName="w-full">
                <View style={{ backgroundColor: theme.panel }} className="flex-col rounded-2xl min-w-[850px] overflow-hidden w-full">
                    <View style={{ backgroundColor: theme.background === "#f8fafc" ? "#f1f5f9" : "#0f172a" }} className="flex-row items-center px-6 py-4">
                        <Text style={{ color: theme.textDark }} className="w-[180px] font-black text-xs uppercase tracking-wider">Subclass Name</Text>
                        <Text style={{ color: theme.textDark }} className="w-[160px] font-black text-xs uppercase tracking-wider px-2">Drug Class Group</Text>
                        <Text style={{ color: theme.textDark }} className="flex-1 font-black text-xs uppercase tracking-wider px-2">Clinical Indication Summary</Text>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        {subclasses.map((item) => (
                            <View key={item.id} className="flex-row items-center px-6 py-4 web:hover:bg-slate-500/5">
                                ...
                                <View className="w-[160px] px-2">
                                    <Text style={{ color: theme.text }} className="bg-slate-500/10 px-2.5 py-0.5 rounded-md font-bold text-xs self-start max-w-full truncate">{item.drug_class_title}</Text>
                                </View>
            ...
            {subclasses.map((item) => (
                <View key={item.id} style={{ backgroundColor: theme.panel }} className={`p-5 rounded-2xl border ${cardBorderClass} shadow-sm flex-col`}>
                    <View className="flex-row justify-between items-start mb-2">
                        ...
                        <Text style={{ color: theme.primary, backgroundColor: theme.primary + "15" }} className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded max-w-[120px] truncate" numberOfLines={1}>
