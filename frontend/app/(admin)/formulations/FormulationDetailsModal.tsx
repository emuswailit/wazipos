...
interface FormulationDetailsModalProps {
    routeItem: FormulationItem | null;
    onClose: () => void;
    theme: any;
...
export default function FormulationDetailsModal({ routeItem, onClose, theme, formatDateHandler, onOpenEditTrigger }: FormulationDetailsModalProps) {
                ...
                <View style={{ backgroundColor: theme.panel, borderBottomColor: theme.border }} className="h-16 w-full border-b px-6 flex-row justify-between items-center z-50 shadow-sm">
                    <View className="flex-row items-center gap-x-3">
                        <View style={{ backgroundColor: theme.primary + "15" }} className="px-2.5 py-1 rounded-md">
                            <Text style={{ color: theme.primary }} className="text-[10px] font-black tracking-widest uppercase">Formulation Spec</Text>
                        </View>
                        <Text style={{ color: theme.text }} className="text-base font-black tracking-tight">Formulation Specifications</Text>
                    </View>
                    ...
                    <View className="w-full max-w-2xl mx-auto flex-col gap-y-6">
                        ...
                        <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm items-start w-full flex-col gap-y-4">
                            <View className="w-full">
                                <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-2">Pharmaceutical Form Type</Text>
                        ...
                        <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm items-start w-full">
                            <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-2">Galenic Properties / Intake medium summary</Text>
                            <Text style={{ color: theme.text }} className="text-sm font-medium leading-relaxed text-left w-full">{routeItem?.description || "No specific formulation descriptors submitted for this record profile parameter."}</Text>
                        </View>
                        ...
                        <View className="flex-row items-center gap-x-4 mt-4 w-full">
                            <TouchableOpacity onPress={handleEditAction} style={{ borderColor: theme.border }} className="flex-1 h-12 rounded-xl border items-center justify-center bg-slate-50 dark:bg-slate-800 active:opacity-70">
                                <Text style={{ color: theme.text }} className="font-black text-xs uppercase tracking-wider">Edit Form Parameters</Text>
                            </TouchableOpacity>
