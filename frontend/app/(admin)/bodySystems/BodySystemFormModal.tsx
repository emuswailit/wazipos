...
const BodySystemFormSchema = Yup.object().shape({
    title: Yup.string().min(3, "Title must be at least 3 characters").required("Body system title is required"),
    description: Yup.string().min(5, "Provide a descriptive structural anatomical layout statement").required("Description summary is required"),
});

export default function BodySystemFormModal({ visible, onClose, isDarkMode, theme, isSubmittingRemote, onSubmitTrigger, initialData, remoteErrors }: BodySystemFormModalProps) {
    const { height: screenHeight } = useWindowDimensions();

    useEffect(() => {
        if (remoteErrors) {
            console.log(`❌ [API Error Matrix] Body System Mutation Failed:`, JSON.stringify(remoteErrors));
        }
    }, [remoteErrors]);
                ...
                <View style={{ backgroundColor: theme.panel, borderBottomColor: theme.border }} className="h-16 w-full border-b px-6 flex-row justify-between items-center shadow-sm">
                    <View className="flex-row items-center gap-x-3">
                        ...
                        <Text style={{ color: theme.text }} className="text-base font-black">
                            {initialData ? "Modify Body System" : "Create Body System"}
                        </Text>
                    </View>
                ...
                <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={true} contentContainerStyle={{ paddingBottom: 40 }} className="flex-1 w-full px-6 py-6">
                    <View className="w-full max-w-2xl mx-auto">
                        <Formik
                            ...
                            validationSchema={BodySystemFormSchema}
                            onSubmit={(values, formikHelpers) => {
                                console.log(`📦 [Payload Monitor] action: "${initialData ? 'UpdateBodySystem' : 'CreateBodySystem'}" | data:`, JSON.stringify(values));
                            ...
                            {({ handleChange, handleBlur, handleSubmit, values, errors, touched }) => (
                                <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm flex-col w-full gap-y-4">
                                    <View className="items-start w-full">
                                        <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-1">System Title</Text>
                                        <TextInput onChangeText={handleChange("title")} onBlur={handleBlur("title")} value={values.title} placeholder="e.g. CARDIOVASCULAR SYSTEM" placeholderTextColor="#64748b" style={{ backgroundColor: theme.background, borderColor: touched.title && errors.title ? "#ef4444" : theme.border, color: theme.text }} className="w-full rounded-xl px-4 h-[42px] border text-sm font-medium outline-none" />
                                        {touched.title && errors.title && <Text className="text-red-500 text-[11px] font-semibold mt-1">{errors.title}</Text>}
                                    </View>
                                    <View className="items-start w-full">
                                        <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-1">Anatomical Functional Description</Text>
                                        <TextInput onChangeText={handleChange("description")} onBlur={handleBlur("description")} value={values.description} placeholder="Specify network boundaries, biological actions, and constraints..." placeholderTextColor="#64748b" multiline numberOfLines={4} style={{ backgroundColor: theme.background, borderColor: touched.description && errors.description ? "#ef4444" : theme.border, color: theme.text, textAlignVertical: 'top' }} className="w-full rounded-xl px-4 py-3 min-h-[100px] border text-sm font-medium outline-none" />
                                    ...
                                    <TouchableOpacity onPress={() => handleSubmit()} disabled={isSubmittingRemote} style={{ backgroundColor: theme.primary }} className="w-full h-12 rounded-xl items-center justify-center shadow-md active:opacity-90 mt-4">
                                        {isSubmittingRemote ? <ActivityIndicator color="#ffffff" size="small" /> : <Text className="text-white font-bold text-sm uppercase tracking-wider">{initialData ? "Update System Metadata" : "Commit System Entry"}</Text>}
                                    </TouchableOpacity>
                                </View>
                            )}
                        </Formik>
