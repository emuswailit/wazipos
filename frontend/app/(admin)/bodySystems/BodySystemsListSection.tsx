...
interface BodySystemsListSectionProps {
    systems: BodySystemItem[];
    isLargeScreen: boolean;
    theme: any;
    cardBorderClass: string;
    formatDateHandler: (dateString: string) => string;
    onOpenDetailsTrigger: (item: BodySystemItem) => void;
    onOpenEditTrigger: (item: BodySystemItem) => void;
}

export default function BodySystemsListSection({
    systems,
    isLargeScreen,
    theme,
    cardBorderClass,
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={true} contentContainerClassName="w-full">
                <View style={{ backgroundColor: theme.panel }} className="flex-col rounded-2xl min-w-[850px] overflow-hidden w-full">
                    <View style={{ backgroundColor: theme.background === "#f8fafc" ? "#f1f5f9" : "#0f172a" }} className="flex-row items-center px-6 py-4">
                        <Text style={{ color: theme.textDark }} className="w-[200px] font-black text-xs uppercase tracking-wider">System Name</Text>
                        <Text style={{ color: theme.textDark }} className="flex-1 font-black text-xs uppercase tracking-wider px-2">Anatomical Description Summary</Text>
