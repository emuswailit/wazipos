...
interface PreparationDetailsModalProps {
    routeItem: PreparationItem | null;
    onClose: () => void;
    theme: any;
...
export default function PreparationDetailsModal({ routeItem, onClose, theme, formatDateHandler, onOpenEditTrigger }: PreparationDetailsModalProps) {
                ...
                <View style={{ backgroundColor: theme.panel, borderBottomColor: theme.border }} className="h-16 w-full border-b px-6 flex-row justify-between items-center z-50 shadow-sm">
                    <View className="flex-row items-center gap-x-3">
                        <View style={{ backgroundColor: theme.primary + "15" }} className="px-2.5 py-1 rounded-md">
                            <Text style={{ color: theme.primary }} className="text-[10px] font-black tracking-widest uppercase">Prep Spec</Text>
                        </View>
                        <Text style={{ color: theme.text }} className="text-base font-black tracking-tight">Preparation Specifications</Text>
                    </View>
                    ...
                    <View className="w-full max-w-2xl mx-auto flex-col gap-y-6">
                        ...
                        <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm items-start w-full flex-col gap-y-4">
                            <View className="w-full">
                                <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-2">Preparation Title</Text>
                                <Text style={{ color: theme.primary }} className="text-xl font-black text-left">{routeItem?.long_title || routeItem?.title}</Text>
                            </View>
                        ...
                        <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm w-full flex-row justify-between gap-4 flex-wrap">
                            <View className="items-start min-w-[140px] flex-1">
                                <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-1">Attached Dosage Formulation</Text>
                                <Text style={{ color: theme.text }} className="text-xs font-bold bg-slate-500/10 px-2.5 py-1 rounded mt-2 self-start uppercase tracking-wide">{routeItem?.formulation_title}</Text>
                            </View>
                            <View className="items-start min-w-[180px] flex-1">
                                <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-1">Active Molecular Compounds</Text>
                                <Text style={{ color: theme.primary }} className="text-xs font-bold bg-blue-500/10 px-2.5 py-1 rounded mt-2 self-start uppercase tracking-wide italic">{routeItem?.generics_string}</Text>
                            </View>
                        </View>
                        ...
                        <View style={{ backgroundColor: theme.panel, borderColor: theme.border }} className="p-6 rounded-2xl border shadow-sm items-start w-full">
                            <Text style={{ color: theme.textDark }} className="text-[10px] uppercase font-black tracking-wider mb-2">Supplemental Dosage Context Guidelines</Text>
                            <Text style={{ color: theme.text }} className="text-sm font-medium leading-relaxed text-left w-full">{routeItem?.description || "No specific systemic indicators or combination guidelines submitted for this record parameters profile."}</Text>
                        </View>
                        ...
                        <View className="flex-row items-center gap-x-4 mt-4 w-full">
                            <TouchableOpacity onPress={handleEditAction} style={{ borderColor: theme.border }} className="flex-1 h-12 rounded-xl border items-center justify-center bg-slate-50 dark:bg-slate-800 active:opacity-70">
                                <Text style={{ color: theme.text }} className="font-black text-xs uppercase tracking-wider">Modify Setup</Text>
                            </TouchableOpacity>
