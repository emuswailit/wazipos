...
export function useRecordCustomerPayment(orderId: string, onSuccessCallback: () => void) {
    ...
    const postPaymentApi = useApi<any>(async (payload: any) => await retailersApi.retailerOrdersAction(payload));
    ...
    const executeSubmitPaymentReceipt = async () => {
        ...
        // 🚀 COMPILING TARGET SPECIFICATION PAYLOAD
        const requestPayload = {
            action: "MakeCustomerOrderPayment",
            payment_method: selectedMethod.id,
            mobile_money_phone: selectedMethod.title === 'MOBILE MONEY' ? mobileNumber.trim() : "",
            customer_order: orderId
        };

        console.log("[OUTBOUND CUSTOMER_PAYMENT_API REQUEST PAYLOAD]:", JSON.stringify(requestPayload, null, 2));
        setPollingError(null);

        if (selectedMethod.title === 'MOBILE MONEY') {
            setIsPollingOpen(true);
            const res = await postPaymentApi.request(requestPayload);
            console.log("[RAW CUSTOMER MOBILE MONEY INITIALIZATION RESPONSE]:", JSON.stringify(res));
        ...
        const res = await postPaymentApi.request(requestPayload);
        console.log("[RAW TRADITIONAL CUSTOMER TRANSACTION LOG RESPONSE]:", JSON.stringify(res));

        if (res?.ok || res?.status === 200 || res?.status === 201 || res?.data?.success === true) {
            const successMsg = res?.data?.response_message || "Traditional customer payment recorded.";
            if (Platform.OS === 'web') {
