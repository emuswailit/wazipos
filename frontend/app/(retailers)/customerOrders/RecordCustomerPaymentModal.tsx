...
// 🔗 Import your authentic synchronized global payment context hook
...
export default function RecordCustomerPaymentModal({ isOpen, orderId, orderRef, onClose, onRefreshParentLedger }: RecordCustomerPaymentModalProps) {
    ...
    // 1. Extract live reactive payment options from the global context provider stream
    const { paymentMethodsList, isPaymentSyncing } = usePaymentMethodsSync();

    const {
        paymentMethods, // Hook state (we will safely overlay this)
        selectedMethod,
        setSelectedMethod,
        mobileNumber,
        setMpesaNumber,
    ...
    } = useRecordCustomerPayment(orderId, () => { if (onRefreshParentLedger) onRefreshParentLedger(); onClose(); });

    // 2. ✅ CRITICAL FIX FOR NATIVE APP: 
    // Force the custom hook's internal empty state to adopt the live context array when the modal opens
    const activePaymentMethods = paymentMethods && paymentMethods.length > 0 ? paymentMethods : paymentMethodsList;

    // Optional: Pre-select the first available cash option once the context array hydrates
    useEffect(() => {
    ...
    console.log("🎯 [Payment Modal Log] Active rendering payment methods:", activePaymentMethods);
            ...
            <Modal visible={isOpen && !isPollingOpen} animationType="fade" transparent={true} onRequestClose={onClose}>
                <View className="flex-1 justify-center items-center p-4 bg-black/50 backdrop-blur-sm">
                    <View style={{ backgroundColor: theme.panel, borderColor: theme.isDarkMode ? '#334155' : '#cbd5e1' }} className="w-full max-w-md rounded-2xl p-6 border shadow-2xl relative">
                        ...
                        {/* Combined loading states to guarantee visual smooth transitions */}
                        {isFetchLoading || (isPaymentSyncing && activePaymentMethods.length === 0) ? (
                            ...
                            <View className="flex-col w-full mb-6">
                                ...
                                <View className="flex-col gap-y-2">
                                    {/* ✅ Render the context array directly */}
