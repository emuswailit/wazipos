...
export function useLineItemMutations(
    setLineItems: React.Dispatch<
        React.SetStateAction<OrderLineItem[]>
    >,
    saveLinesToStorage: (
        lines: OrderLineItem[]
    ) => Promise<void>
    ...
    const handleSelectProduct = (
        ...
        // Server UUID goes in `selectedProduct`; local Dexie `id`
        ...
        setLineItems((prev) => {
            const next = prev.map((item) =>
                item.id === id
                    ? {
                        ...item,
                        selectedProduct: receiptId
                            ? String(receiptId)
                            : null,
                        selectedProductTitle:
                            prod?.title || '',
                        searchQuery: prod?.title || '',
