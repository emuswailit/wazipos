// components/wholesalers/productsRequests/ViewRequestDetailsModal.tsx
//
// Read-only modal that shows a product request's line items and
// any offers already made on them.
//
// Available for every request status — a wholesaler who has
// already responded (ACKNOWLEDGED, PARTIALLY_FULFILLED, etc.) can
// reopen the request to review what they offered.
//
// No editing, no submit. Just a scrollable summary.
...
const ViewRequestDetailsModal: React.FC<Props> = ({
    visible,
    request,
    onClose,
}) => {
            ...
            <View className="flex-1 bg-black/40 items-center justify-center p-3">
                ...
                >
                    ...
                    <View
                        ...
                        style={{
                            borderBottomColor: theme.textDark + '22',
                        }}
                    >
...
const LineCard: React.FC<{
    item: ProductRequestSummaryLineItem;
    index: number;
}> = ({ item, index }) => {
    const { theme, isDarkMode } = useAuth();
    const borderColor = isDarkMode ? '#334155' : '#e2e8f0';
        ...
        >
            ...
            >
                ...
                <View className="flex-row items-center flex-wrap gap-1.5 mt-2">
                    ...
                    {item.status_display || item.status ? (
                        <View
                            className="px-2 py-0.5 rounded-md"
                            style={{ backgroundColor: lineStatus.bg }}
                        >
                            <Text
                                className="uppercase tracking-wide"
                                style={{
                    ...
                    >
                        ...
                        >
                            {item.requested_quantity ?? 0} requested
                        </Text>
                    </View>
            ...
            >
                ...
                {offers.length === 0 ? (
                    ...
                    >
                        No offers have been made on this line.
                    </Text>
                ) : (
                    offers.map((o) => (
...
const OfferRow: React.FC<{ offer: ProductRequestOffer }> = ({
    ...
    const total =
        Number.isFinite(unitPrice) && unitPrice > 0
            ? unitPrice * (Number(offer.offered_quantity) || 0)
            : 0;
    ...
    return (
        ...
        >
            ...
            {offer.retailer_response_note?.trim() ? (
                ...
                >
                    Retailer: {offer.retailer_response_note}
                </Text>
            ) : null}
