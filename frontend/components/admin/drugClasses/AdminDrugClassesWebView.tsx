// components/admin/drugClasses/AdminDrugClassesWebView.tsx
...
const COLUMNS: { label: string; flex: number }[] = [
    { label: 'Drug Class', flex: 2.0 },
    { label: 'Category', flex: 1.8 },
    { label: 'Clinical Brief', flex: 3.0 },
    { label: 'Created', flex: 1.2 },
    { label: 'Actions', flex: 1.4 },
];

type Props = AdminDrugClassesSharedProps;

function rowKey(item: DrugClassItem): string {
    return item.id || String(Math.random());
}

export function AdminDrugClassesWebView({
    query,
    setQuery,
    onRefresh,
    refreshing,
                    ...
                    {items.length === 0 ? (
                        emptyComponent ?? (
                            <View className="p-8 items-center">
                                ...
                                >
                                    No drug classes match the filters.
                                </Text>
                            </View>
                        )
                    ) : (
...
export default AdminDrugClassesWebView;